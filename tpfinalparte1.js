//definitivo Manejo de ARREGLOS, FUNCIONES, IMAGENES y SONIDO
...
function mostrarpantalla(pantalla, texto, botones = []) {
  ...
  // Mostrar el texto de la pantalla
  text(textos[pantalla - 1], 0, 100, width, height);

  // Dibujar los botones si existen
  for (let boton of botones) {
    botonRectangular(boton[0], boton[1], boton[2], 200, 60); // Botón con ancho y alto fijos
  }
}

function mousePressed() {
  if (pantalla === 1) {
    if (suprficie(mouseX, mouseY, 80, 400, 200, 60)) {
      pantalla = 2;
      sonido1.play();
